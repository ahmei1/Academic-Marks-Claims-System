...
import { CheckCircle, XCircle, AlertTriangle, User, FileText, Download } from 'lucide-react';
    ...
    const handleOpenGrading = (course) => {
        ...
        const rows = enrolledStudentIds.map(stId => {
            ...
            return {
                ...
                assignment: existingMark?.assignment || 0,
                markId: existingMark?.id || null // If null, we create; if exists, we update (optional)
            };
        });
    ...
    const handleSaveGrades = async () => {
        ...
        for (const row of gradingStudents) {
            const markData = {
                ...
                total: row.cat + row.fat + row.assignment
            };
                        ...
                        return (
                            <div key={course.id} className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 p-6 transition-all hover:shadow-md">
                                ...
                                <div className="bg-slate-50 dark:bg-slate-700/50 rounded-lg p-3 text-center mb-4">
                                    <div className="flex items-center justify-center gap-2 text-slate-700 dark:text-slate-300 font-semibold">
                    ...
                    <div className="bg-white dark:bg-slate-800 w-full max-w-4xl max-h-[90vh] overflow-y-auto rounded-xl shadow-2xl border border-slate-200 dark:border-slate-700 flex flex-col">
                        ...
                        <div className="p-6">
                            {gradingStudents.length === 0 ? (
                                <div className="text-center py-8 text-slate-500 dark:text-slate-400">No students enrolled yet.</div>
                            ) : (
                                    ...
                                    <table className="w-full text-sm text-left">
                                        <thead className="text-xs text-slate-700 dark:text-slate-300 uppercase bg-slate-50 dark:bg-slate-700/50">
                                            <tr>
                                                ...
                                                <th className="px-4 py-3 w-24">CAT (30)</th>
                                                <th className="px-4 py-3 w-24">FAT (60)</th>
                                                <th className="px-4 py-3 w-24">Assgn (10)</th>
                                                <th className="px-4 py-3 w-20 rounded-r-lg">Total</th>
                                            </tr>
                                        </thead>
                                            ...
                                            {gradingStudents.map(student => (
                                                <tr key={student.studentId} className="hover:bg-slate-50 dark:hover:bg-slate-700/30 transition-colors">
                                                    ...
                                                    <td className="px-4 py-3">
                                                        <input type="number" className="w-full px-2 py-1 rounded border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-800 text-center focus:ring-2 focus:ring-blue-500 outline-none transition-all"
                                                    ...
                                                    <td className="px-4 py-3">
                                                        <input type="number" className="w-full px-2 py-1 rounded border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-800 text-center focus:ring-2 focus:ring-blue-500 outline-none transition-all"
                                                            value={student.assignment} onChange={(e) => handleGradeChange(student.studentId, 'assignment', e.target.value)} />
                                                    </td>
                                                    <td className="px-4 py-3 font-bold text-slate-900 dark:text-white text-center">
                                                        {student.cat + student.fat + student.assignment}
                                                    </td>
                                                </tr>
                                            ))}
                                        </tbody>
                        ...
                        <div className="p-6 border-t border-slate-200 dark:border-slate-700 flex justify-end gap-3 bg-slate-50 dark:bg-slate-800/50 rounded-b-xl">
                            ...
                            <button onClick={handleSaveGrades} className="px-6 py-2 bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-lg shadow-sm transition-colors">Save All Marks</button>
                        </div>
                    </div>
                </div>
            )}
